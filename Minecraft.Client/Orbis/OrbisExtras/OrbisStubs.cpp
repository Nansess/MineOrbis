...
static const char *s_rawSaveDirPath = "/data/minecraft/Saves";
int32_t hBGMAudio;
...
const char* getRawSaveDirPath()
{
	return s_rawSaveDirPath;
}
...
static void ResolveOrbisFilePath(const char *lpFileName, char *filePath, size_t filePathSize)
{
	...
	else if(strncmp(lpFileName, "Saves", 5) == 0 || strncmp(lpFileName, "FakeTMSPP", 9) == 0)
	{
		snprintf(filePath, filePathSize, "/data/minecraft/%s", lpFileName);
	}
	else
	{
...
bool OrbisEnsureDirectoryExists(const char *path)
{
	...
	ResolveOrbisFilePath(path, resolvedPath, sizeof(resolvedPath));

	if(sceFiosDirectoryExistsSync(NULL, resolvedPath))
	{
		return true;
	}

	char partialPath[512];
	memset(partialPath, 0, sizeof(partialPath));

	size_t length = strnlen(resolvedPath, sizeof(partialPath) - 1);
	if(length == 0 || length >= sizeof(partialPath))
	{
		return false;
	}
	...
	{
		partialPath[i] = resolvedPath[i];
		if(partialPath[i] == '/' && i > 0)
		{
			partialPath[i] = 0;
			if(!sceFiosDirectoryExistsSync(NULL, partialPath))
			{
				int err = sceFiosDirectoryCreateSync(NULL, partialPath);
				if(err != SCE_FIOS_OK)
				{
					return false;
				}
...
{
	char filePath[512];
	ResolveOrbisFilePath(lpFileName, filePath, sizeof(filePath));
	...
	{
	case CREATE_ALWAYS:
		openParams.openFlags |= SCE_FIOS_O_CREAT | SCE_FIOS_O_TRUNC;	break;
	case CREATE_NEW:
	...
	case TRUNCATE_EXISTING:
		openParams.openFlags |= SCE_FIOS_O_TRUNC;	break;
	}
...
{
	return OrbisEnsureDirectoryExists(lpPathName) ? TRUE : FALSE;
}
...
BOOL DeleteFileA(LPCSTR lpFileName)
{
	char filePath[512];
	ResolveOrbisFilePath(lpFileName, filePath, sizeof(filePath));
	return sceFiosDeleteSync(NULL, filePath) == SCE_FIOS_OK;
}
...
BOOL MoveFileA(LPCSTR lpExistingFileName, LPCSTR lpNewFileName)
{
	...
	char newPath[512];
	ResolveOrbisFilePath(lpExistingFileName, existingPath, sizeof(existingPath));
	ResolveOrbisFilePath(lpNewFileName, newPath, sizeof(newPath));
	return sceFiosRenameSync(NULL, existingPath, newPath) == SCE_FIOS_OK;
}
