...
void UIScene_PauseMenu::PerformActionSaveGame()
{
	...
	// Folder-backed ORBIS saves do not use the native save-exists query.
#if defined(__ORBIS__)
	bool bUsingRawFolderSaves = app.GetLoadSavesFromFolderEnabled();
	bool bSaveExists = true;
	C4JStorage::ESaveGameState result = C4JStorage::ESaveGame_Idle;
	if(!bUsingRawFolderSaves)
	{
		result = StorageManager.DoesSaveExist(&bSaveExists);
	}
#else
	bool bSaveExists;
