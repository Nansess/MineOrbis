...
#ifdef __ORBIS__
static std::string GetOrbisSaveFilename(const std::wstring &fileName)
{
...
static std::string GetOrbisSaveRelativePath(const std::wstring &fileName)
{
	std::string saveName = GetOrbisSaveFilename(fileName);
	if(saveName.empty())
	{
		return saveName;
	}
...
ConsoleSaveFileOriginal::ConsoleSaveFileOriginal(const wstring &fileName, LPVOID pvSaveData /*= NULL*/, DWORD dFileSize /*= 0*/, bool forceCleanSave /*= false*/, ESavePlatform plat /*= SAVE_FILE_PLATFORM_LOCAL*/)
{
	...
	{
#ifdef __ORBIS__
		std::string savePath = GetOrbisSaveRelativePath(m_fileName);
		if(!savePath.empty())
		{
...
#elif (defined __PS3__ || defined __PSVITA__ || defined _DURANGO || defined _WINDOWS64)
