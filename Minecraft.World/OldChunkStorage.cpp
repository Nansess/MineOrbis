...
static void oldChunkStorage_itow36(int value, wchar_t *buffer, size_t bufferCount)
{
	...
	bool negative = currentValue < 0;
	unsigned long long magnitude = negative ? (unsigned long long)(-currentValue) : (unsigned long long)currentValue;
	wchar_t temp[64];
	size_t index = 0;

	do
	{
		temp[index++] = digits[magnitude % 36];
		magnitude /= 36;
	}
	while(magnitude != 0 && index < (sizeof(temp) / sizeof(temp[0])));

	size_t writeIndex = 0;
	if(negative && writeIndex + 1 < bufferCount)
	{
		buffer[writeIndex++] = L'-';
	}

	while(index > 0 && writeIndex + 1 < bufferCount)
	{
		buffer[writeIndex++] = temp[--index];
	}
...
File OldChunkStorage::getFile(int x, int z)
{
	...
	oldChunkStorage_itow36(x, xRadix36, sizeof(xRadix36) / sizeof(xRadix36[0]));
	oldChunkStorage_itow36(z, zRadix36, sizeof(zRadix36) / sizeof(zRadix36[0]));
	swprintf(name,MAX_PATH_SIZE,L"c.%ls.%ls.dat",xRadix36,zRadix36);
	oldChunkStorage_itow36(x & 63, path1, sizeof(path1) / sizeof(path1[0]));
	oldChunkStorage_itow36(z & 63, path2, sizeof(path2) / sizeof(path2[0]));
#else
	_itow(x,xRadix36,36);
	_itow(z,zRadix36,36);
	swprintf(name,MAX_PATH_SIZE,L"c.%ls.%ls.dat",xRadix36,zRadix36);
